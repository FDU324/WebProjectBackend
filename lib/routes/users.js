...
router.put('/modifyPassword', function (req, res, next) {
    User.findOne({
        where: {username : req.body.username}
    }).then(user => {
        if ((user === null) || (user.password !== req.body.oldPassword)) {
            console.log('password modifiy failed.' + user.username);
            res.send({data: 'wrong'});
        }
        else {
            User.update({
                ...
                console.log('password modified.' + user.username);
                res.send({data: 'success'});
            })
        }
    }).catch(err => {
