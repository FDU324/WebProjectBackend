...
router.put('/modifyNickname', function (req, res, next) {
    TestUser.update({
        ...
        //currentUsers[req.body.username].emit('connect', 'success');
        //console.log(currentUsers);
        //console.log(currentUsers[req.body.username]);
        res.send({data: 'success', user: JSON.stringify(user)});
    }).catch(function (err) {
        console.log('failed: ' + err);
        res.send({data: err});
        ...
        if (user === null) {
            ...
            Friend.findOne({
                ...
                if (friend === null) {
                    ...
                    currentUsers[req.query.friendUsername].emit('receiveNewFriendApply', username.myUsername);
                    res.send({data: 'success'});
                } else {
